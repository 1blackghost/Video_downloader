...
class Y_D:
    ...
    def start_download(self, obj):
        obj.download("static",filename=self.filename)

    def download(self, res, data,filename):
        '''
        ...
        if data[1]=="video":
            get = self.yt.streams.filter(res=res, mime_type=data[0], progressive=data[2]).first()
        elif data[1]=="audio":
            get = self.yt.streams.filter(abr=res, mime_type=data[0]).first()
        
        t = threading.Thread(target=self.start_download, args=(get,))
        t.start()
